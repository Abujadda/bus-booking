<?php 
	...
	if(isset($_POST['add_bus'])){
		...
		$busNo = mysqli_real_escape_string($isah, $_POST['bus_no']);
		$busName = mysqli_real_escape_string($isah, $_POST['bus_name']);
		$route = mysqli_real_escape_string($isah, $_POST['route']);
		$depeature = mysqli_real_escape_string($isah, $_POST['depeature']);
		$capacity = mysqli_real_escape_string($isah, $_POST['capacity']);
		$arrival = mysqli_real_escape_string($isah, $_POST['arrival']);
		$amount = mysqli_real_escape_string($isah, $_POST['amount']);


		$add = mysqli_query($isah, "INSERT INTO `bus_schedules` (bus_no, bus_name, route, depeature, arrival_time, capacity, amount, status) VALUES ('$busNo', '$busName', '$route', '$depeature', '$arrival', '$capacity', '$amount', '1')");
		if($add){
