<?php 
	...
	if(isset($_POST['add_user'])){
		...
		$m_status = mysql_real_escape_string($isah, $_POST['m_status']);
		$gender = mysqli_real_escape_string($isah, $_POST['gender']);
		$city = mysqli_real_escape_string($isah, $_POST['city']);
		$bus_name = mysqli_real_escape_string($isah, $_POST['bus_name']);
		$bus_no = mysqli_real_escape_string($isah, $_POST['bus_no']);
		$dlaciences = mysqli_real_escape_string($isah, $_POST['dlaciences']);
		...
		$add = mysqli_query($isah, "INSERT INTO `drivers` (fname, lname, email, phone, m_status, gender, city, bus_name, bus_no, address, dlicence,  status) VALUES ('$fname', '$lname', '$email', '$phone', '$m_status', '$gender', '$city', '$bus_name', '$bus_no', '$address', '$dlaciences', '1')");
		if($add){
