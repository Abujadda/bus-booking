<?php 
	...
	$loadPay = mysqli_query($isah, "SELECT * FROM payment_booking WHERE booking_id='$check' AND status='1'");
	...
	if(isset($_POST['checkUser'])){
		...
		$ticket = mysqli_query($isah,"SELECT * FROM payment_booking WHERE booking_id='$check' AND status='1'");
		$row = mysqli_num_rows($ticket);

		if($row>0){

			$loadData = mysqli_query($isah, "SELECT * FROM payment_booking WHERE booking_id='$check' AND status='1'");
			$view = mysqli_fetch_array($loadData);
