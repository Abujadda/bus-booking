	...
	$loadBook = mysqli_query($isah, "SELECT * FROM user_booking WHERE email='".$_SESSION['email']."'");
	...
	if(isset($_POST['payment'])){
		...
		$cname = mysqli_real_escape_string($isah, $_POST['cname']);
		$cnumber = mysqli_real_escape_string($isah,$_POST['cnumber']);
		$ccv = mysqli_real_escape_string($isah, $_POST['ccv']);
		$cexpired = mysqli_real_escape_string($isah, $_POST['cexpired']);
		$cpin = mysqli_real_escape_string($isah, $_POST['cpin']);
		$route = mysqli_real_escape_string($isah, $_POST['route']);
		$amount = mysqli_real_escape_string($isah, $_POST['amount']);
		$ctype = mysqli_real_escape_string($isah, $_POST['ctype']);
		$bname = mysqli_real_escape_string($isah, $_POST['bname']);
		...
		$payment = mysqli_query($isah, "INSERT INTO `payment_booking` (fname, lname, email, cname, cnumber, ccv, booking_id, bus_no, route, amount, ctype, bname, payment_date, payment_id, status) VALUES ('$fname', '$lname', '".$_SESSION['email']."', '$cname', '$cnumber', '$ccv', '".$bookRow['booking_id']."', '".$bookRow['bus_no']."', '$route', '$amount', '$ctype', '$bname', '".date('d-m-Y')."', '".md5(5)."', '1')");
		if($payment){
